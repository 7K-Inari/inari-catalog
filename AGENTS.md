...
- Release flow: release-please manifest mode (PR-only, `skip-github-release: true`) with one component per `packages/<name>` → per-package tags `<name>-vX.Y.Z`. Release PRs are merged manually; `release.yml` then invokes the signed OCI publish (`workflow_call`, never tag-push triggers), creates tags + GitHub Releases only after every publish succeeded (a failed publish leaves no tag, so the next push retries), and finally regenerates `catalog.yaml` and pushes it as the signed index artifact `ghcr.io/7k-inari/catalog/index:latest` (consumed by inari-server's `INARI_CATALOG_OCI_INDEX_REF`). See `docs/release-process.md`.
